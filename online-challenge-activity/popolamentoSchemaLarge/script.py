...
def main():
	outF = open("popolamentoGioco.sql", "w")
	outD = open("popolamentoDado.sql", "w")
	outS = open("popolamentoSfida.sql", "w")
	...
	for x in lettere:
		...
		for y in lettere:
			...
			for z in numeri:
				...
				line = "INSERT INTO Gioco VALUES ('" + str(x) + str(y) + str(z) + "', 3, 'sfondo.png', '" + random.choice(setGiochi) + "' , 'Neque porro quisquam est qui dolorem ipsum quia dolor sit amet, consectetur, adipisci velit.');"
	...
	for num in range(11, 9999):
		...
		line3 = "INSERT INTO Sfida VALUES (" + str(num) + ", '08-" + str(random.choice(month)) + "-2021 21:56:32.5', " + str(random.randint(0, 600)) +  ",'" + str(sceltaGioco) + "');"
